...
CREATE INDEX idx_soldproduct_productid ON soldProduct (product_id);
...
-- 1. Primero creamos o reemplazamos la función
CREATE
OR REPLACE FUNCTION set_pay_status() RETURNS TRIGGER AS $ $ BEGIN -- Comparamos si el estado es Reparado y el pago es completo
IF NEW.repair_status = 'Reparado'
...
-- 2. Creamos el TRIGGER (Esto es lo que hace que la función se ejecute)
-- Se ejecutará automáticamente antes de cada INSERT o UPDATE
CREATE
OR REPLACE TRIGGER trigger_device_pay BEFORE
INSERT
      OR
UPDATE
...
CREATE INDEX idx_history_spare_parts ON historyTicket USING GIN (spare_parts);