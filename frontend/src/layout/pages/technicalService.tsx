...
import ReadQR from "@/components/common/readQR";
...
import { parseFormDevice } from "@/components/utils/parseFormDevice";
...
const TechnicalService = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // convert images to webp before sending to backend
      const convertedFiles = await Promise.all(
        deviceImgs.map((file) => convertToWebp(file)),
      );
      // parse form data to match backend structure
      const formDevice = parseFormDevice(deviceData, convertedFiles);
      let result = null;
      if (isEditing) {
        result = await updateDevice(editingDeviceId, formDevice);
      }
      if (!isEditing) {
        result = await createDevice(formDevice);
      }
      if (!result.success) {
  ...
  const handlerEditDevice = (d: TechnicalServiceEntry, files: File[]) => {
    if (d.output_status) {
  ...
  const handleDeviceDetail = (device: TechnicalServiceEntry) => {
    setDeviceDetail(device);
    setOpenDetail(true);
  };
  ...
  return (
    <div className="w-full h-full p-2 text-white">
      ...
      <Modal
        title="Tomar foto del dispositivo"
        isOpen={isOpenTakePhoto}
        onClose={() => setOpenTakePhoto(false)}
      >
        <TakePhoto
          addImage={(image) => {
